...
public List<ToeicQuestionResponse> createToeicQuestions(List<ToeicQuestionCreateRequest> toeicQuestionCreateRequests, Long examId, MultipartFile[] images) {
  ...
  for (int i = 0; i < toeicQuestionCreateRequests.size(); i++) {
      ...
      // Chuyển đổi từ request thành entity ToeicQuestion
      ToeicQuestion toeicQuestion = toeicQuestionMapper.toToeicQuestion(toeicQuestionCreateRequest);
      toeicQuestion.setToeicExam(toeicExam);  // Gán ToeicExam vào câu hỏi

      // Nếu có ảnh, lưu ảnh vào thư mục và gán đường dẫn ảnh vào câu hỏi
      if (images != null && images.length > i) {
          ...
          try {
              ...
              toeicQuestion.setIamge("/images/" + imageName);  // Đường dẫn ảnh
          } catch (IOException e) {
              throw new AppException(ErrorCode.FILE_UPLOAD_ERROR, "Error uploading image", e);
          }
      }
      ...
      // Chuyển đổi câu hỏi đã lưu thành response
      ToeicQuestionResponse response = toeicQuestionMapper.toToeicQuestionResponse(toeicQuestion);
      response.setImage(toeicQuestion.getIamge());  // Gán đường dẫn ảnh vào response
      responses.add(response);  // Thêm response vào danh sách trả về
  }

  // Trả về danh sách câu hỏi đã tạo và trả về đường dẫn ảnh
  return responses;
}
